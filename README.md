...
1~5단계 파일에는 문법 연습을 위한 Mock 데이터가 남아 있고, 현재 실행 화면은 Axios로 실제 날씨를 요청합니다. 통신에 실패한 도시만 Mock 데이터로 대체해 화면 전체가 멈추지 않도록 했습니다.
...
교수님 예제에 있던 서울, 수원, 부산 외에 제주, 인천, 광주 데이터를 추가했습니다. 도시마다 `theme` 값을 함께 넣고 카드를 클릭했을 때 이 값에 맞는 장면을 보여주도록 했습니다.
...
`v-for`는 배열의 각 데이터를 같은 화면 구조로 반복해서 만들 때 사용하는 문법이라고 이해했습니다. 서울, 수원, 부산뿐 아니라 제주, 인천, 광주를 추가하더라도 카드 HTML을 도시 수만큼 복사하지 않는 편이 수정하기 쉽다고 판단했습니다. 그래서 개인 도시 데이터와 비·눈 입자를 `v-for`로 출력했습니다. 도시에는 배열 순서 대신 고유한 `id`를 `:key`로 사용했고, 입자에도 각각의 ID를 지정했습니다.
...
`computed`는 기존 반응형 데이터를 이용해 새로운 결과를 계산하고, 의존하는 값이 바뀔 때 다시 계산하는 기능입니다. 산책 추천은 선택 도시의 기온, 강우 확률, 미세먼지, 초미세먼지를 조합해야 하고 빨래 추천은 강우 확률과 습도를 조합해야 합니다. 별도의 결과 값을 직접 수정하는 것보다 날씨 데이터에서 항상 같은 기준으로 계산되는 기능이므로 `computed`가 어울린다고 판단했습니다. 도시가 바뀌면 시간별 예보와 추천 결과가 연속해서 다시 계산되도록 구성했습니다.
...
`props`는 부모가 가진 데이터를 자식에게 전달하는 단방향 통신이라고 이해했습니다. 날씨 애니메이션에는 `theme`, 위험기상 경고에는 `status`, 생활 추천 카드에는 도시 이름과 계산 결과가 필요합니다. 이 컴포넌트들이 부모의 데이터를 직접 수정할 필요는 없으므로 `props`로 받는 구조가 적합하다고 판단했습니다. 같은 컴포넌트에 다른 도시 데이터를 전달해도 화면을 재사용할 수 있다는 점이 좋았습니다.
...
Router는 URL과 화면을 연결하고, 새로고침하거나 주소를 공유해도 같은 화면 상태로 접근할 수 있게 하는 기능이라고 이해했습니다. 도시 상세 화면은 여섯 도시가 같은 구조를 사용하고 데이터만 다르므로 도시마다 View를 만드는 것보다 `/weather/:cityId` 동적 경로가 어울린다고 판단했습니다. `WeatherCard`의 상세보기 이벤트에서 `router.push()`로 도시 ID를 전달하고, 상세 View에서는 `route.params.cityId`로 해당 도시와 시간별 예보를 찾습니다.
...
UI Library는 버튼이나 입력창의 모양만 가져오는 도구가 아니라, 반복되는 화면 상태와 상호작용을 검증된 컴포넌트로 표현하는 도구라고 이해했습니다. 도시 검색에는 사용자가 입력을 쉽게 취소할 수 있는 `clearable` 속성의 `ElInput`이 어울린다고 판단했습니다. 기존 `update-query` emit은 그대로 유지해 입력값이 부모의 검색 `computed`로 전달되게 했습니다.

위험기상 경고와 통신 오류는 일반 문장보다 사용자가 바로 알아차려야 하는 정보이므로 아이콘과 상태 색상을 제공하는 `ElAlert`를 사용했습니다. 위험기상 경고는 사용자가 실수로 닫아 놓지 않도록 `closable`을 false로 설정했습니다. 반대로 카드와 추천은 계속 읽는 일반 정보이므로 `ElCard`를 사용해 경고와 시각적인 우선순위를 구분했습니다.

실제 API는 응답을 기다리는 시간이 있기 때문에 빈 화면 대신 `ElSkeleton`을 보여주는 편이 적합하다고 판단했습니다. 검색 결과가 없는 경우에는 오류가 아니라 정상적인 빈 상태이므로 경고 Alert 대신 `ElEmpty`를 사용했습니다. 같은 “화면에 데이터가 없음”이라도 원인이 로딩, 검색 결과 없음, 통신 실패 중 무엇인지에 따라 다른 UI를 선택했습니다.

시간별 예보는 같은 필드가 12번 반복되고 행끼리 비교해야 하므로 `ElTable`을 사용했습니다. 기온 열에는 일반 `prop`만 지정하지 않고 slot에서 `formatTemperature()`를 호출해 Pinia의 섭씨·화씨 설정이 표에도 적용되게 했습니다. 즉 UI Library를 적용한 뒤에도 이전 단계의 Store 기능이 끊기지 않도록 데이터 흐름을 유지했습니다.
...
- 전체 파일을 ESLint와 Oxlint로 검사했고 과제 실행 코드에 오류가 없는 것을 확인했습니다.
- `npm run build`가 Vite의 기본 프로덕션 모드로 실행되도록 수정했습니다.
- Vue DevTools 플러그인은 개발 서버에서만 등록하고 제출용 빌드에서는 제외했습니다.
- GitHub의 `main` Branch에 Push하면 ESLint 검사, 빌드, GitHub Pages 배포가 순서대로 실행되는 GitHub Actions Workflow를 작성했습니다.
...
Vite의 빌드는 개발 중인 Vue 파일을 브라우저가 바로 받을 수 있는 정적 파일로 변환하는 과정이라고 이해했습니다. 개발 서버에서 잘 실행되는 것만으로는 제출용 파일이 정상인지 판단할 수 없기 때문에 프로덕션 빌드와 Preview를 따로 확인하기로 했습니다.

GitHub Actions는 제 컴퓨터가 아닌 독립된 환경에서 같은 명령을 반복 실행하는 자동화 기능입니다. 제 컴퓨터에서만 우연히 성공한 결과를 올리는 것보다, Push할 때마다 `npm ci`, ESLint, Build를 다시 검사하는 방식이 제출 상태를 확인하기에 좋다고 판단했습니다. 이 과정이 성공한 파일만 GitHub Pages에 배포되도록 Build Job과 Deploy Job을 분리했습니다.

API 키를 `.env`로 분리하면 코드에 키 문자열을 직접 작성하지 않을 수 있습니다. 다만 Vite의 `VITE_` 환경변수는 브라우저 번들에 포함되므로 완전한 비밀 저장 방법은 아니라는 점도 확인했습니다. 이번 실습에서는 저장소에 키를 직접 남기지 않고, 필요하면 키를 교체할 수 있게 환경변수로 관리했습니다.

## 이벤트와 데이터 적용 기록

| 이벤트 | 사용한 데이터 | 처리 내용 |
| --- | --- | --- |
| 검색창 `@input` | `searchQuery` | 입력한 한글 도시 이름을 상태에 저장하고 화면에 표시 |
| 날씨 카드 `@click` | 선택한 도시 객체 | 선택 도시 문구와 날씨 장면 변경 |
| 상세보기 `@click.stop` | 도시 이름과 상태 | 카드 이벤트 전파를 막고 날씨 내용을 알림으로 표시 |
| `v-for` 렌더링 | `weatherList` | 각 도시의 ID를 key로 사용해 날씨 카드 생성 |
| 검색어 변경 | `searchQuery` | `computed`가 도시 목록을 계산하고 `watchEffect`가 결과 안내문 갱신 |
| 선택 도시 변경 | `selectedWeather`와 시간별 Mock 예보 | `watch`가 변경 안내문을 만들고 `computed`가 추천과 장면을 다시 계산 |
| `update-query` emit | 자식 검색창의 입력값 | 부모의 `searchQuery` 변경 |
| `select-card` emit | 선택한 도시 객체 | 부모에서 장면, 추천, 선택 문구 변경 |
| 상세보기 `click-detail` emit | 도시 ID | `router.push()`로 `/weather/:cityId` 이동 |
| 단위변경 버튼 `@click` | Pinia의 `unit` | 메인·상세·시간별 기온을 섭씨 또는 화씨로 표시 |
| 위치 확인 버튼 `@click` | 브라우저 좌표 | `locationStore`의 좌표·상태·오류 값 변경 |
| 화면 진입 및 다시 불러오기 | 도시 좌표 | Axios로 실제 날씨·예보·대기질 요청 후 기존 화면 형식으로 변환 |
| 위치 권한 성공 | Pinia의 위도·경도 | 좌표 기반 Axios 요청 후 현재 위치 추천과 날씨 장면 갱신 |
...
Element Plus를 적용하면서 UI Library와 기존 Vue 기능은 서로 대신하는 관계가 아니라는 점을 느꼈습니다. `ElInput`을 사용해도 입력값은 emit으로 부모에 전달해야 하고, `ElTable`을 사용해도 단위 변환에는 Pinia와 함수가 필요했습니다. 라이브러리는 화면 표현을 맡고 데이터 상태와 이벤트 흐름은 기존 Vue 코드가 맡도록 구분했습니다.

## Troubleshooting

### 데스크톱에서 앱이 화면 왼쪽 절반에만 표시된 문제

처음 개발 서버를 실행했을 때 날씨 화면이 가운데 넓게 표시되지 않고 화면 왼쪽 절반에만 배치되는 문제가 생겼습니다. 날씨 컴포넌트의 너비 문제라고 생각했지만 개발자 도구로 확인해 보니 Vue 기본 템플릿의 `main.css`에 `#app`을 2열 Grid로 만드는 코드가 남아 있었습니다. 이번 과제는 하나의 날씨 대시보드를 보여주는 구조이기 때문에 데스크톱 레이아웃을 단일 열로 변경하고 `.app-container`의 좌우 margin을 `auto`로 수정했습니다. 수정 후 1280px 화면에서 앱이 900px 너비로 가운데 표시되는 것을 확인했습니다.

### Mockup만 실행했는데 빌드 파일이 크게 생성된 문제

1단계에서는 `WeatherMockup.vue`만 사용했는데 초기 빌드 결과가 약 1MB로 표시되었습니다. 화면에 보이지 않는 컴포넌트도 모두 빌드되는지 궁금해 `main.js`를 확인했고, 이후 단계에서 사용할 Router, Pinia, Element Plus가 처음부터 전부 등록되어 있는 것을 찾았습니다. 단계별 차이를 확인하려면 1단계에서 필요하지 않은 플러그인을 먼저 제외하는 편이 낫다고 생각했습니다. `main.js`에는 Vue 앱 생성 코드만 남기고 Router, Pinia, Element Plus는 각 실습 단계에서 다시 추가하도록 수정했습니다.

수정 전에는 JS 빌드 파일이 약 1,003KB였지만 외부 플러그인을 제외한 후에는 약 64KB로 줄었습니다. 화면에 직접 보이지 않더라도 앱 시작 시 등록한 라이브러리는 빌드 결과에 포함될 수 있다는 것을 확인했습니다.

### Watcher를 넣었지만 화면 기능에는 사용되지 않았던 문제

2단계를 다시 확인하면서 `watch`와 `watchEffect`가 상태 변화를 `console.log()`로 출력하기만 하고 있다는 것을 발견했습니다. 문법을 실행해 본 것은 맞지만 사용자가 이용하는 개인 기능에는 영향을 주지 않아, 필수 기술을 기능에 적용했다는 설명이 약하다고 생각했습니다. 먼저 도시 변경에는 이전 값과 새 값이 필요하므로 `watch(selectedWeather)`가 적합하다고 판단했습니다. 도시가 바뀌면 “서울에서 제주로 변경했고 장면과 추천을 다시 계산했다”는 안내를 화면에 표시하도록 수정했습니다.

검색 안내는 검색어와 `computed`로 계산한 목록 개수를 동시에 사용하므로 `watchEffect`로 두 값을 자동 추적하게 했습니다. 검색 전에는 전체 도시 수, 검색 성공 시에는 결과 수, 실패 시에는 결과 없음 문구를 표시했습니다. 수정 후 제주를 입력했을 때 검색 결과 1개가 표시되고, 제주 카드를 선택했을 때 도시 변경 문구와 태풍 경고 및 추천 결과가 함께 바뀌는 것을 확인했습니다. Watcher는 값을 관찰하는 것에서 끝나는 것이 아니라, 변경 뒤 필요한 부수 효과가 있을 때 사용해야 한다는 점을 이해했습니다.

### 교수님 예제의 API 키가 코드에 직접 들어 있던 문제

교수님 Axios 예제를 참고하면서 API 키가 문자열로 컴포넌트 안에 들어 있는 것을 확인했습니다. 그대로 사용하면 제출 파일을 보는 사람에게 키가 노출되고, 나중에 키를 바꿀 때 여러 파일을 수정해야 한다는 문제가 있다고 생각했습니다. 실제 키는 `.env`의 `VITE_OPENWEATHER_API_KEY`에서 읽고, 저장소에는 변수 이름만 적힌 `.env.example`을 남기도록 수정했습니다. 키가 없는 현재 환경에서는 요청 자체를 보내지 않고 Open-Meteo 실제 데이터로 동작하게 하여 불필요한 401 오류도 막았습니다.

### 서로 다른 API 응답을 기존 컴포넌트가 바로 사용하지 못한 문제

Open-Meteo는 `temperature_2m`, `weather_code`처럼 시간별 값을 각각의 배열로 주고, 대기질 API는 PM10과 PM2.5를 별도 배열로 반환했습니다. 기존 컴포넌트는 시간 하나마다 기온·강우·습도·미세먼지가 들어 있는 객체 배열을 사용하므로 응답을 그대로 넣을 수 없었습니다. `weatherApi.js`에서 시간 문자열을 기준으로 두 API 데이터를 합치고 기존의 `temp`, `rainProbability`, `humidity`, `pm10`, `pm25`, `status` 형식으로 변환했습니다. 수정 후 기존 추천 함수의 코드를 바꾸지 않아도 제주 실제 예보에서 산책 시간과 빨래 가능 여부가 계산되는 것을 확인했습니다.

### 위치 권한 완료 전에 날씨 요청이 시작될 수 있었던 문제

기존 `requestCurrentLocation()`은 브라우저 콜백 안에서 좌표를 저장할 뿐 완료 결과를 반환하지 않았습니다. 이 상태에서는 View가 좌표 저장이 끝난 시점을 알 수 없어 현재 위치 날씨 요청 순서를 연결하기 어려웠습니다. Pinia action이 `Promise<boolean>`을 반환하도록 바꾸고, View에서 `await`한 결과가 성공일 때만 Axios 요청을 실행하도록 수정했습니다. 권한 거부 시에는 API를 호출하지 않고 Store의 오류 문구만 표시되도록 분리했습니다.

### Element Plus 전체 등록 후 빌드 파일이 다시 커진 문제

처음에는 교수님 예제처럼 `app.use(ElementPlus)`를 사용해 전체 컴포넌트를 등록했습니다. 기능은 정상 동작했지만 빌드 결과에서 메인 JavaScript가 약 1,057KB가 되었고 500KB보다 큰 청크 경고가 나타났습니다. 화면에서 실제로 사용하는 컴포넌트가 아홉 종류뿐인데 전체 라이브러리를 포함하는 것은 낭비라고 생각했습니다.

`main.js`에서 `ElAlert`, `ElButton`, `ElCard`, `ElEmpty`, `ElInput`, `ElSkeleton`, `ElTable`, `ElTableColumn`, `ElTag`만 가져와 반복 등록하도록 수정했습니다. 수정 후 메인 JavaScript가 약 392KB로 줄었고 큰 청크 경고도 사라졌습니다. UI Library를 사용하는 것과 전체 라이브러리를 모두 포함하는 것은 다르다는 점을 확인했습니다. 다만 Element Plus 내부의 `@vueuse/core` 주석에 대한 Rolldown 경고는 의존성 코드에서 발생했고 빌드는 성공했기 때문에 8단계에서 버전과 함께 다시 확인할 예정입니다.

### 제출용 빌드가 개발 모드로 설정된 문제

8단계에서 `package.json`을 확인하니 `npm run build`가 `vite build --mode development`로 실행되고 있었습니다. 개발 중에 사용하는 환경을 제출용 산출물에도 명시하는 구조였기 때문에 Vite의 기본 프로덕션 빌드인 `vite build`로 수정했습니다. 또한 Vue DevTools를 개발 서버에서만 등록하도록 분기했습니다. 수정 후 새로 빌드하고 `dist/index.html`과 Route 복구용 `dist/404.html`이 같이 생성되는지 확인했습니다.
...
구체적인 단계별 계획과 결정 사항은 [`PROJECT_PLAN.md`](./PROJECT_PLAN.md)에 기록하고 있습니다.
